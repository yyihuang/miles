...
kernel_cake_gdn_chunk_train_99ba0855e2578a159dc1(__nv_bfloat16* __restrict__ q, __nv_bfloat16* __restrict__ k, float* __restrict__ g, __nv_bfloat16* __restrict__ beta, float* __restrict__ beta32_out, __nv_bfloat16* __restrict__ qn, __nv_bfloat16* __restrict__ kn, float* __restrict__ rstd_q, float* __restrict__ rstd_k, float* __restrict__ g_cs, int* __restrict__ chunk_start, int* __restrict__ chunk_len, int num_heads, int num_v_heads, int normalize_qk)
{
    ...
    if (warp == 0) {
        ...
        if (lane_0 < n_valid) {
            ...
            beta32_out[gate_index_lo] = (float)beta[gate_index_lo];
        }
        if (n_valid > lane_0 + 32) {
            g_cs[gate_index_hi] = g_hi * 1.4426950408889634f;
            beta32_out[gate_index_hi] = (float)beta[gate_index_hi];
        }
    }
}
